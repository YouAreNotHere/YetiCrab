...
const AttractionsList = ({
  ...
  const [visibleAttractionsFilter, setVisibleAttractionsFilter] = useState<
  ...
  const visitedAttractionsCount = useMemo(
    () =>
  ...
  const visibleAttractions = useMemo(() => {
    ...
    switch (visibleAttractionsFilter) {
      case 'visited':
        filtered = currentAttractions.filter(
    ...
    if (showOnlyRated)
      filtered = filtered.filter((item) => item.rating.length > 0);
  ...
  return (
    <section className="attractions-form">
      <h1 className="attractions-form__header">Достопримечательности</h1>

      <div className="attractions-form__outter-wrapper">
        <div className="attractions-form__inner-wrapper">
          <Text variant="body-3">Всего: {currentAttractions.length}</Text>
          <Text variant="body-3">Посетили: {visitedAttractionsCount}</Text>
        ...
        <div className="filter-and-sort">
          ...
          <Select
            ...
            onUpdate={(value) =>
              setVisibleAttractionsFilter(value[0] as TFilter)
            }
          />
